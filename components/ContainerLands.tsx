...
export const ContainerLands = () => {
  ...
  return (
    <>
      <Landing isNew={true} title={"Trending Now"} movie={trending} AddorOut={"ADD List"}/>
       {MyList.length > 0 && (
            <Landing isNew={false} title={"My List"} movie={MyList} AddorOut={"remove List"} />
          )}
      <Landing isNew={false} title={"Music"} movie={Music} AddorOut={"ADD List"} />
      <Landing isNew={false} title={"Terror"} movie={terror}  AddorOut={"ADD List"}/>
      <Landing isNew={false} title={"Programas TV"} movie={tv}  AddorOut={"ADD List"}/>
      <Landing isNew={false} title={"Comedy"} movie={comedy}  AddorOut={"ADD List"}/>
    </>
  );
};
